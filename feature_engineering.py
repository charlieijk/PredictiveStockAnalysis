...
class FeatureEngineer:
    ...
    def __init__(
        ...
        scaling_method: str = "standard",   # 👈 NEW
    ) -> None:
        ...
        self.scaling_method = scaling_method  # 👈 store it
        ...
        if scaling_method not in scaler_map:
            raise ValueError(
                f"Unknown scaling_method '{scaling_method}'. "
                f"Choose from {list(scaler_map.keys())}."
            )

        self.scaler = scaler_map[scaling_method]
